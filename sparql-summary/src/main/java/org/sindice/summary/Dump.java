...
public class Dump {
  ...
  private void dumpTriple(String s, String p, String o)
  throws IOException {
    _output.write(s);
    _output.write(" ");
    _output.write(p);
   ...
   * @throws IOException
   */
  private long dumpRDFNodeCollection(String nodeType,
                                     String cardinality,
                                     String domain)
  throws IOException {
    Resource s = null;
    ...
    for (String pair : nodeType.split(" ")) {
      ...
      s = new Resource((AnalyticsVocab.ANALYTICS_PREFIX + "ec" + Hash.getLong(domain +
                                                                              nodeType +
                                                                              element))
      .replace('-', 'n'), false);
      ...
      for (DomainVocab vocabWord : DomainVocab.values()) {
        if (count++ == vocabValue) {
          dumpTriple(o.toN3(), p.toN3(), "<" + vocabWord.type() + ">");
          break;
        }
      }
    ...
    for (String pair : nodeType.split(" ")) {
      ...
      if (!nodeList.contains(element)) {
        ...
        s = new Resource((AnalyticsVocab.ANALYTICS_PREFIX.toString() + "node" + hash)
        .replace('-', 'n'), false);
        p = new Resource(AnalyticsVocab.LABEL.toString(), false);
        o = new Resource((AnalyticsVocab.ANALYTICS_PREFIX.toString() + "ec" + Hash
        .getLong(domain + nodeType + element)).replace('-', 'n'), false);
        dumpTriple(s.toN3(), p.toN3(), o.toN3());
        nodeList.add(element);
      }
    }
  ...
  throws QueryEvaluationException, IOException {
    ...
    if (bindingSet.hasBinding("label")) {
      // label
      long hash = dumpRDFNodeCollection(bindingSet.getValue("label").toString(), bindingSet
      .getValue("cardinality").toString(), _sndDomain);
      ...
      if (_domain.equals("sindice.com")) {
        o = new Resource(AnalyticsVocab.DOMAIN_URI_PREFIX.toString() + _domain, false);
      } else {
  ...
  throws QueryEvaluationException, IOException {
    if (bindingSet.hasBinding("label")) {
      ...
      // Don't put the type prdicate in the output
      isNotType = true;
      for (DomainVocab defType : DomainVocab.values()) {
        if (defType.type().equals(bindingSet.getValue("label").toString())) {
          isNotType = false;
          break;
        }
      }
      if (isNotType) {
        ...
        s = new Resource((AnalyticsVocab.ANALYTICS_PREFIX.toString() + "edge" + Hash
        .getLong(_domain + _nodeCounter +
                 bindingSet.getValue("label").toString())).replace('-', 'n'), false);
        ++_nodeCounter;
        p = new Resource(AnalyticsVocab.LABEL.toString(), false);
        ...
        o = new Resource((AnalyticsVocab.ANALYTICS_PREFIX.toString() + "node" + Hash
        ...
        if ((bindingSet.getValue("target") == null) ||
          ...
          o = new Resource((AnalyticsVocab.ANALYTICS_PREFIX.toString() + "bc" + randomGenerator
          .nextInt()).replace('-', 'n'), false);
        } else {
          p = new Resource(AnalyticsVocab.EDGE_TARGET.toString(), false);
          o = new Resource((AnalyticsVocab.ANALYTICS_PREFIX.toString() + "node" + Hash
          .getLong(_sndDomain + bindingSet.getValue("target").toString()))
          .replace('-', 'n'), false);
        }
        dumpTriple(s.toN3(), p.toN3(), o.toN3());
  ...
  public void openRDF(String outputFile, String domain) {
    ...
    try {
      // make sure the path to the output file exists, or create if it is not
      final File dir = new File(outputFile).getParentFile();
      dir.mkdirs();
      // Create file
