	...
	private long dumpRDFNodeCollection(String nodeType, String cardinality,
		...
		for (String pair : nodeType.split(" ")) {
			...
			if (!nodeList.contains(element)) {
				...
				try {
					o = new Resource((URLDecoder.decode(element, "UTF-8")),
					        true);
				} catch (Exception e) {
			...
			o = new Resource(
			        (AnalyticsVocab.TYPE.toString() + Hash.getLong(domain
			                + nodeType + type + element)).replace('-', 'n'),
			        false);
		...
		for (String pair : nodeType.split(" ")) {
			...
			if (!nodeList.contains(element)) {
				...
				s = new Resource((AnalyticsVocab.ANALYTICS_PREFIX.toString()
				        + "node" + hash).replace('-', 'n'), false);
				p = new Resource(AnalyticsVocab.LABEL.toString(), false);
				o = new Resource(
				        (AnalyticsVocab.ANALYTICS_PREFIX.toString() + "ec" + Hash
				                .getLong(domain + nodeType + element))
				                .replace('-', 'n'), false);
				dumpTriple(s.toN3(), p.toN3(), o.toN3());
				nodeList.add(element);
			}
	...
	public void dumpRDFNode(BindingSet bindingSet)
		...
		if (bindingSet.hasBinding("label")) {
			...
			long hash = dumpRDFNodeCollection(bindingSet.getValue("label")
			        .toString(),
			        bindingSet.getValue("cardinality").toString(), _sndDomain);
			...
			// cardinality
			p = new Resource(AnalyticsVocab.CARDINALITY.toString(), false);
			o = new Resource(bindingSet.getValue("cardinality").toString(),
			        true);
	...
	public void dumpRDFPred(BindingSet bindingSet)
		...
		if (bindingSet.hasBinding("label")) {
			...
			if (isNotType) {
				...
				s = new Resource((AnalyticsVocab.ANALYTICS_PREFIX.toString()
				        + "edge" + Hash.getLong(_domain + _nodeCounter
				        + bindingSet.getValue("label").toString())).replace(
				        '-', 'n'), false);
				++_nodeCounter;
				p = new Resource(AnalyticsVocab.LABEL.toString(), false);
				o = new Resource(bindingSet.getValue("label").toString(),
				        false);
				dumpTriple(s.toN3(), p.toN3(), o.toN3());

				p = new Resource(AnalyticsVocab.CARDINALITY.toString(), false);
				o = new Resource(
				        bindingSet.getValue("cardinality").toString(), true);
				dumpTriple(s.toN3(), p.toN3(), o.toN3());
				...
				o = new Resource((AnalyticsVocab.ANALYTICS_PREFIX.toString()
				        ...
				        + bindingSet.getValue("source").toString())).replace(
				        '-', 'n'), false);
				dumpTriple(s.toN3(), p.toN3(), o.toN3());
				...
				if ((bindingSet.getValue("target") == null)
				        || (bindingSet.getValue("target").toString()
				                .equals("\"\""))) {
					// blank collection
					p = new Resource(AnalyticsVocab.EDGE_TARGET.toString(),
					        false);
					o = new Resource(
					...
					p = new Resource(AnalyticsVocab.EDGE_TARGET.toString(),
					        false);
					o = new Resource(
					        (AnalyticsVocab.ANALYTICS_PREFIX.toString()
					                + "node" + Hash.getLong(_sndDomain
					                + bindingSet.getValue("target").toString()))
					                .replace('-', 'n'), false);
				}
				...
				// Dummy node
				o = new Resource("\""
				        + AnalyticsVocab.BLANK_NODE_COLLECTION.toString()
				        + "\"", true);
				dumpTriple(s.toN3(), p.toN3(), o.toN3());

				// published In
				p = new Resource(AnalyticsVocab.EDGE_PUBLISHED_IN.toString(),
				        false);
				if (_domain.equals("sindice.com")) {
					o = new Resource(
					        AnalyticsVocab.DOMAIN_URI_PREFIX.toString()
					                + _domain, false);
				} else {
					o = new Resource(_domain, false);
				}
