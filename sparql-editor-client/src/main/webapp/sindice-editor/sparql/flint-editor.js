...
function FlintEditor(container, imagesPath, config) {
	...
	try {
		...
		// 				createSidebar.updateProperties(endpointItem.getDomain(),epItem);
		// 				createSidebar.updateClasses(endpointItem.getDomain(),epItem);
		...
		function() {
			...
			//createSidebar.updateOMIM(datasetItems.getItem(datasetItems.getEndpoint()));
			//createSidebar.updateChEMBL(datasetItems.getItem(datasetItems.getEndpoint()));
			//createSidebar.updateCTD(datasetItems.getItem(datasetItems.getEndpoint()));
			//createSidebar.updateSERD(datasetItems.getItem(datasetItems.getEndpoint()));
			//createSidebar.updateGOA(datasetItems.getItem(datasetItems.getEndpoint()));
		    }
		}
	    );
		...
		try {
			...
			success: function(data) {
			  resultsArea.setResults(JSON.stringify(data));
			}
		    });*/
		    $.ajax({
		...
		if (e.keyCode == 32 && (e.ctrlKey || e.metaKey) && !e.altKey) {
		    //console.log(g_isInAutocompletionPanel);
		    //console.log ("CTRL+SPACE /o/");
		    e.stop();
		...
		//function showContextss(sel.options[sel.selectedIndex].value,tkposs){
		function showContext(){
			...
			*/
			...
			// var tokenAct = cm.getTokenAt(cursor);
			// //console.log(tokenAct.string);
			// //console.log(tokenAct.priorToken.string);
			// cursor.ch -= 1;
			// while (tokenAct.string === cm.getTokenAt(cursor).string
			//        || ((cm.getTokenAt(cursor).string !== "a")
			// 	   && (cm.getTokenAt(cursor).string
			...
			//     else {
			// 	cursor.line -= 1;
			// 	var l = cm.getLine(cursor.line);
			// 	cursor.ch = l.length;
			//     }
			// }
			// tokenAct = cm.getTokenAt(cursor);
		...
		function clearCS() {
			...
			else if (punct.exec(possibles[i]))
			{
			    ...
			    /*if (/^</.test(start)) { 
			      ...
			      var q = cm.getValue();
			      var outputss;
			      var d = endpointItem.getDomain();
			      $.ajax({
			      url: config.endpoints[0].uri,
			      type: 'post',
			      data: "data=autocomplete&domain="+encodeURIComponent(d)+"&query="+encodeURIComponent(q)+"&cursorLine="+cur.line+"&cursorPosition="+cur.ch,
			      headers: {"Accept": "application/sparql-results-json"},
			      crossDomain: true,
			      dataType: 'jsonp',
			      //processData: false,
			      ...
			      success: function(data) {
			      outputss = data;
			      for (var j = 0; j < outputss.results.bindings.length;  j++) {
			      maybeAddCS("<"+outputss.results.bindings[j]+">");
			      }
	...
	try {
	    ...
	    //toolbarItems.push(new FlintToolbarItem("Dataset Recommendation", "Show dataset recommendation panel", "Globe_24x24.png", true, function() {}, true));
...
//	if(jQuery().datasetRecommendation) {
//	    $('#flint-toolbar-8').datasetRecommendation({
//	    	deltaV:35
//	    });
//	}else{
//		alert("Dataset Recommendation Plugin Not Loaded");
//	}
	    for (var i = 0; i < toolbarItems.length; i++) {
...
//				}
	...
	// function displayOMIM() {
	//     $('#flint-sidebar-content').text("");
	//     if (activeDataItem) {
	// 	if (activeDataItem.omim != null) {
	// 	    try {
	// 		var sampleText = "";
	// 		for (var i = 0; i < activeDataItem.omim.length; i++) {
	// 		    var query = activeDataItem.omim[i].query;
	// 		    query = query.replace(/</g, "&lt;");
	// 		    query = query.replace(/>/g, "&gt;");
	// 		    if (activeDataItem.omim[i].name !== "") {
	// 		    sampleText += "<div class='flint-sample' title=''Click to insert sample into editing pane'><h3>" + activeDataItem.omim[i].name + "</h3><p>" + activeDataItem.omim[i].description + "</p><pre class='flint-sample-content'>" + query + "</pre></div>";
	// 		    } else {
	// 			sampleText += "<div class='flint-sample' title=''Click to insert sample into editing pane'><p>" + activeDataItem.omim[i].description + "</p><pre class='flint-sample-content'>" + query + "</pre></div>";
	// 		    }
	// 		}
	// 		sampleText = "<div id='flint-samples'>" + sampleText + "</div>";
	// 		$('#flint-sidebar-content').append(sampleText);
	...
	// 				}
	// 			    });
	// 			    editor.getConfirmDialog().show("Insert OMIM Query", "<p>Are you sure you want to abandon the current text?</p>");
	// 			}
	// 			e.stopPropagation();
	// 		    }
	// 		);	
	...
	// function displayChEMBL() {
	//     $('#flint-sidebar-content').text("");
	//     if (activeDataItem) {
	// 	if (activeDataItem.chembl != null) {
	// 	    try {
	// 		var sampleText = "";
	// 		for (var i = 0; i < activeDataItem.chembl.length; i++) {
	// 		    var query = activeDataItem.chembl[i].query;
	// 		    query = query.replace(/</g, "&lt;");
	// 		    query = query.replace(/>/g, "&gt;");
	// 		    if (activeDataItem.chembl[i].name !== "") {
	// 		    sampleText += "<div class='flint-sample' title=''Click to insert sample into editing pane'><h3>" + activeDataItem.chembl[i].name + "</h3><p>" + activeDataItem.chembl[i].description + "</p><pre class='flint-sample-content'>" + query + "</pre></div>";
	// 		    } else {
	// 			sampleText += "<div class='flint-sample' title=''Click to insert sample into editing pane'><p>" + activeDataItem.chembl[i].description + "</p><pre class='flint-sample-content'>" + query + "</pre></div>";
	// 		    }
	// 		}
	// 		sampleText = "<div id='flint-samples'>" + sampleText + "</div>";
	// 		$('#flint-sidebar-content').append(sampleText);
	...
	// 				}
	// 			    });
	// 			    editor.getConfirmDialog().show("Insert CHEMBL Query", "<p>Are you sure you want to abandon the current text?</p>");
	// 			}
	// 			e.stopPropagation();
	// 		    }
	// 		);	
	...
	//     $('#flint-sidebar-content').text("");
	//     if (activeDataItem) {
	// 	if (activeDataItem.ctd != null) {
	// 	    try {
	// 		var sampleText = "";
	// 		for (var i = 0; i < activeDataItem.ctd.length; i++) {
	// 		    var query = activeDataItem.ctd[i].query;
	// 		    query = query.replace(/</g, "&lt;");
	// 		    query = query.replace(/>/g, "&gt;");
	// 		    if (activeDataItem.ctd[i].name !== "") {
	// 		    sampleText += "<div class='flint-sample' title=''Click to insert sample into editing pane'><h3>" + activeDataItem.ctd[i].name + "</h3><p>" + activeDataItem.ctd[i].description + "</p><pre class='flint-sample-content'>" + query + "</pre></div>";
	// 		    } else {
	// 			sampleText += "<div class='flint-sample' title=''Click to insert sample into editing pane'><p>" + activeDataItem.ctd[i].description + "</p><pre class='flint-sample-content'>" + query + "</pre></div>";
	// 		    }
	// 		}
	// 		sampleText = "<div id='flint-samples'>" + sampleText + "</div>";
	// 		$('#flint-sidebar-content').append(sampleText);
	...
	// 				}
	// 			    });
	// 			    editor.getConfirmDialog().show("Insert CTD Query", "<p>Are you sure you want to abandon the current text?</p>");
	// 			}
	// 			e.stopPropagation();
	// 		    }
	// 		);	
	...
	// function displaySERD() {
	//     $('#flint-sidebar-content').text("");
	//     if (activeDataItem) {
	// 	if (activeDataItem.serd != null) {
	// 	    try {
	// 		var sampleText = "";
	// 		for (var i = 0; i < activeDataItem.serd.length; i++) {
	// 		    var query = activeDataItem.serd[i].query;
	// 		    query = query.replace(/</g, "&lt;");
	// 		    query = query.replace(/>/g, "&gt;");
	// 		    if (activeDataItem.serd[i].name !== "") {
	// 		    sampleText += "<div class='flint-sample' title=''Click to insert sample into editing pane'><h3>" + activeDataItem.serd[i].name + "</h3><p>" + activeDataItem.serd[i].description + "</p><pre class='flint-sample-content'>" + query + "</pre></div>";
	// 		    } else {
	// 			sampleText += "<div class='flint-sample' title=''Click to insert sample into editing pane'><p>" + activeDataItem.serd[i].description + "</p><pre class='flint-sample-content'>" + query + "</pre></div>";
	// 		    }
	// 		}
	// 		sampleText = "<div id='flint-samples'>" + sampleText + "</div>";
	// 		$('#flint-sidebar-content').append(sampleText);
	...
	// 				}
	// 			    });
	// 			    editor.getConfirmDialog().show("Insert SERD Query", "<p>Are you sure you want to abandon the current text?</p>");
	// 			}
	// 			e.stopPropagation();
	// 		    }
	// 		);	
	...
	//     $('#flint-sidebar-content').text("");
	//     if (activeDataItem) {
	// 	if (activeDataItem.goa != null) {
	// 	    try {
	// 		var sampleText = "";
	// 		for (var i = 0; i < activeDataItem.goa.length; i++) {
	// 		    var query = activeDataItem.goa[i].query;
	// 		    query = query.replace(/</g, "&lt;");
	// 		    query = query.replace(/>/g, "&gt;");
	// 		    if (activeDataItem.goa[i].name !== "") {
	// 		    sampleText += "<div class='flint-sample' title=''Click to insert sample into editing pane'><h3>" + activeDataItem.goa[i].name + "</h3><p>" + activeDataItem.goa[i].description + "</p><pre class='flint-sample-content'>" + query + "</pre></div>";
	// 		    } else {
	// 			sampleText += "<div class='flint-sample' title=''Click to insert sample into editing pane'><p>" + activeDataItem.goa[i].description + "</p><pre class='flint-sample-content'>" + query + "</pre></div>";
	// 		    }
	// 		}
	// 		sampleText = "<div id='flint-samples'>" + sampleText + "</div>";
	// 		$('#flint-sample-content').append(sampleText);
	// 		$('.flint-goa-content').click(
	// 		    function(e) {
	// 			var okay = true;
	// 			var goa = $(this);
	// 			if (editor.getCodeEditor().getValue() != "") {
	...
	// 				}
	// 			    });
	// 			    editor.getConfirmDialog().show("Insert GOA Query", "<p>Are you sure you want to abandon the current text?</p>");
	// 			}
	// 			e.stopPropagation();
	// 		    }
	// 		);	
	...
	function showTab(tabName, id) {
	    ...
	    //else if (tabName == "OMIM") {displayOMIM();}
	    //else if (tabName == "ChEMBL") {displayChEMBL();}
	    //else if (tabName == "CTD") {displayCTD();}
	    //else if (tabName == "SERD") {displaySERD();}
	    //else if (tabName == "GOA") {displayGOA();}
	    else displaySparql();
	}
	...
	this.display = function(container) {
	    ...
	    $('#' + container).append("<div id='flint-sidebar'>"
				      ...
				      //+ "<li id='flint-sidebar-omim'' title='View sample queries for the current dataset'>OMIM</li>"
				      //+ "<li id='flint-sidebar-chembl'' title='View sample queries for the current dataset'>ChEMBL</li>"
				      //+ "<li id='flint-sidebar-ctd'' title='View sample queries for the current dataset'>CTD</li>"
				      //+ "<li id='flint-sidebar-serd'' title='View sample queries for the current dataset'>SERD</li>"
				      //+ "<li id='flint-sidebar-goa'' title='View sample queries for the current dataset'>GOA</li>"
		...
		function() {
			...
			if ($('#flint-sidebar').width() > 50) {
			    ...
			    //$('#flint-omim').css("white-space", "nowrap");
			    //$('#flint-chembl').css("white-space", "nowrap");
			    //$('#flint-ctd').css("white-space", "nowrap");
			    //$('#flint-serd').css("white-space", "nowrap");
			    //$('#flint-goa').css("white-space", "nowrap");
	    ...
	    // $('#flint-sidebar-omim').click(
	    // 	function(e) {
	    // 	    showTab("OMIM", $(this).attr("id"));
	    // 	    e.stopPropagation();
	    // 	}
	    // );
	    // $('#flint-sidebar-chembl').click(
	    // 	function(e) {
	    // 	    showTab("ChEMBL", $(this).attr("id"));
	    // 	    e.stopPropagation();
	    // 	}
	    // );
	    // $('#flint-sidebar-ctd').click(
	    // 	function(e) {
	    // 	    showTab("CTD", $(this).attr("id"));
	    // 	    e.stopPropagation();
	    // 	}
	    // );
	    // $('#flint-sidebar-serd').click(
	    // 	function(e) {
	    // 	    showTab("SERD", $(this).attr("id"));
	    // 	    e.stopPropagation();
	    // 	}
	    // );
	    // $('#flint-sidebar-goa').click(
	    // 	function(e) {
	    // 	    showTab("GOA", $(this).attr("id"));
	    // 	    e.stopPropagation();
	    // 	}
	    // );
	}
	...
	// this.updateOMIM = function(datasetItem) {
	//     activeDataItem = datasetItem;
	//     if (activeTab == "OMIM") displayOMIM();
	// }

	// this.updateChEMBL = function(datasetItem) {
	//     activeDataItem = datasetItem;
	//     if (activeTab == "ChEMBL") displayChEMBL();
	// }

	// this.updateCTD = function(datasetItem) {
	//     activeDataItem = datasetItem;
	//     if (activeTab == "CTD") displayCTD();
	// }

	// this.updateSERD = function(datasetItem) {
	//     activeDataItem = datasetItem;
	//     if (activeTab == "SERD") displaySERD();
	// }

	// this.updateGOA = function(datasetItem) {
	//     activeDataItem = datasetItem;
	//     if (activeTab == "GOA") displayGOA();
	// }
	...
	this.updateProperties = function(datasetDomain, datasetItem) {
		...
		if (datasetItem.properties == null) {
		    ...
		    this.showActiveTab();
		    var paramsData = {};
		    //paramsData[config.defaultEndpointParameters.queryParameters.query] = "PREFIX an: <http://sindice.com/vocab/analytics#> SELECT  DISTINCT ?label  FROM <http://sindice.com/analytics-12-09-2011> WHERE { ?node an:domain ?domain .  ?node an:property ?prop .  ?prop an:property ?prop2 .  ?prop2 an:label ?label .  }  LIMIT 1000";
		    $.ajax({
			url: config.endpoints[0].uri,
			type: 'post',
